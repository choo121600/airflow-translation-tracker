  ...
  async calculateLanguageCoverage(owner, repo, targetLanguage, customPath = null) {
    ...
    try {
      ...
      const result = {
        ...
        isEstimated: !targetData.isFullyParsed // Flag for estimated values
      };

      const estimationNote = result.isEstimated ? ' (estimated)' : '';
      console.log(`📊 Coverage for ${targetLanguage}: ${result.coverage}%${estimationNote} (${targetKeyCount}/${baseKeyCount} keys)`);
