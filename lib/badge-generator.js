class BadgeGenerator {
  ...
  generateSVG(label, message, color, options = {}) {
    ...
    // Center text within each section's rectangle
    const labelX = (labelWidth + logoWidth + 12) / 2; // Center in label section
    const messageX = labelWidth + logoWidth + 12 + (messageWidth + 18) / 2; // Match with messageBgWidth padding 
  ...
  createSVGTemplate(width, style) {
    const cornerRadius = style === 'flat' ? 3 : 3; // Always use rounded corners
  ...
  createSVGContent(label, message, color, labelWidth, messageWidth, labelX, messageX, logoWidth, logo, style) {
    ...
    // Center percentage text with equal padding on both sides
    const messagePadding = 12; // Equal padding on left and right of percentage
    const messageBgWidth = messageWidth + (messagePadding * 2);
    const centeredMessageX = messageBgX + messageBgWidth / 2; // Perfect center
    ...
    <g fill="#fff" text-anchor="middle" font-family="${this.fontFamily}" text-rendering="geometricPrecision" font-size="${this.fontSize}">
      ...
      <text aria-hidden="true" x="${centeredMessageX}" y="15" fill="#010101" fill-opacity=".3">${this.escapeXml(message)}</text>
      <text x="${centeredMessageX}" y="14" fill="#fff">${this.escapeXml(message)}</text>
    </g>`;
