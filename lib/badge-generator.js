class BadgeGenerator {
  ...
  generateSVG(label, message, color, options = {}) {
    ...
    const messageSectionWidth = messageWidth + (messagePadding * 2); // Consistent padding
  ...
  <defs>
    ...
    <clipPath id="r">
      <rect width="${width}" height="${this.height}" rx="${cornerRadius}" ry="${cornerRadius}"/>
    </clipPath>
  </defs>
  <g clip-path="url(#r)">
    {{CONTENT}}
  ...
  createSVGContent(label, message, color, labelWidth, messageWidth, labelX, messageX, logoWidth, logo, style) {
    ...
    // Corner radius matching the template
    const cornerRadius = style === 'flat' ? 4 : 6;
    
    let content = `
    <rect width="${labelBgWidth}" height="${this.height}" fill="#4a4a4a"/>
