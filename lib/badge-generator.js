class BadgeGenerator {
  ...
  generateSVG(label, message, color, options = {}) {
    ...
    const messagePadding = 14; // Balanced padding for percentage section
    
    // Calculate section dimensions
    const labelSectionWidth = labelWidth + logoWidth + labelPadding;
    const messageSectionWidth = messageWidth + (messagePadding * 2);
    const totalWidth = labelSectionWidth + messageSectionWidth;
    
    // Calculate precise center positions for text
    const labelX = labelSectionWidth / 2; // Perfect center of label section
