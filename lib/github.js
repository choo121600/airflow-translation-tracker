...
class GitHubClient {
  ...
  async detectI18nStructure(owner, repo, customPath = null) {
    ...
    // Smart path prioritization based on success patterns
    let searchPaths;
    if (customPath) {
      searchPaths = [customPath];
    } else {
      // Reordered based on actual success rates to minimize API calls
      searchPaths = [
        'src/locales',          // Most successful path for modern projects
        'public/locales',       // Next.js/React apps (very common)
        'locales',              // Traditional simple structure
        'public/i18n/locales',  // Apache Airflow style (nested structure)
        'src/i18n',             // Alternative naming convention
        'i18n',                 // Root level (less common)
      ];
    }
    ...
    for (const basePath of searchPaths) {
      try {
        ...
        if (structure.isValidI18n) {
          ...
          console.log(`Structure pattern: ${structure.pattern}, Languages: ${structure.languages.join(', ')}`);
          
          // Cache the successful structure immediately with extended TTL
          cache.setCachedStructure(owner, repo, structure, customPath || '');
          
          // Early return - no need to check other paths
          return structure;
        } else {
          console.log(`Invalid structure at ${owner}/${repo}/${basePath} (${contents.length} items found)`);
        }
      } catch (error) {
