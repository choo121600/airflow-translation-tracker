...
class CacheManager {
  constructor() {
    // Much more aggressive caching to minimize API calls
    this.fileCache = new NodeCache({ stdTTL: 3600 }); // 1 hour (translation files change rarely)
    this.coverageCache = new NodeCache({ stdTTL: 1800 }); // 30 minutes (coverage can be cached longer)
    this.etagCache = new NodeCache({ stdTTL: 3600 }); // 1 hour for ETags
    this.structureCache = new NodeCache({ stdTTL: 7200 }); // 2 hours (structure rarely changes)
    this.fallbackCache = new NodeCache({ stdTTL: 0 }); // Never expire - for fallback data
    this.requestCache = new NodeCache({ stdTTL: 300 }); // 5 minutes for identical requests
  }
