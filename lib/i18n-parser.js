  ...
  async parseRepository(owner, repo, customPath = null) {
    try {
      ...
      if (structure.pattern === 'lang-dir') {
        // 🚀 Ultra-simple approach: just count keys per language
        console.log(`🚀 Key counting approach: ${structure.languages.length} languages`);
        
        // Process all languages with minimal API calls - just count keys
        const countPromises = structure.languages.map(async (lang) => {
          try {
            console.log(`🔢 Counting keys for: ${lang}`);
            ...
            if (Array.isArray(files) && files.length > 0) {
              ...
              // Count keys in all JSON files for this language
              for (const jsonFile of jsonFiles) {
                try {
                  const content = await this.github.getFileContent(owner, repo, jsonFile.path);
                  const keyCount = this.countJsonKeys(content.content);
                  totalKeys += keyCount;
                  console.log(`📊 ${jsonFile.name}: ${keyCount} keys`);
                } catch (error) {
                  console.log(`⚠️ Failed to count keys in ${jsonFile.name}: ${error.message}`);
                }
              }
              
              keyCounts[lang] = totalKeys;
            ...
            console.log(`❌ ${lang}: failed to count keys - ${error.message}`);
            keyCounts[lang] = 0;
          }
        });
        ...
        // Simple key counting for file-based structure
        console.log(`🔢 Counting keys in ${structure.languages.length} language files`);
