  ...
  async parseRepository(owner, repo, customPath = null) {
    try {
      ...
      if (structure.pattern === 'lang-dir') {
        ...
        const langPromises = structure.languages.map(async (lang) => {
        ...
        const langPromises = structure.languages.map(async (lang) => {
          ...
          return { lang, translation: this.parseJsonContent(content.content) };
        });
    ...
    try {
      ...
      if (Array.isArray(files)) {
        ...
        const filePromises = jsonFiles.map(async (file) => {
          ...
          return { namespace, translation: this.parseJsonContent(content.content) };
        });
