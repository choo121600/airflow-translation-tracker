  ...
  async parseRepository(owner, repo, customPath = null) {
    try {
      ...
      if (structure.pattern === 'lang-dir') {
        ...
        // Step 2: Parse other languages and compare with base language keys
        ...
        const parsePromises = otherLanguages.map(async (lang) => {
          try {
            ...
            const langTranslation = await this.parseLanguageDirectory(owner, repo, `${structure.basePath}/${lang}`);
            const langKeys = this.getAllKeysFromTranslation(langTranslation);
