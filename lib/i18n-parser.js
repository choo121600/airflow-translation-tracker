  ...
  async parseRepository(owner, repo, customPath = null) {
    try {
      ...
      if (structure.pattern === 'lang-dir') {
        // Optimized batch processing - limit concurrent API calls
        console.log(`Processing ${structure.languages.length} languages in batches`);
        const batchSize = 3; // Process 3 languages at a time to reduce API load
        
        for (let i = 0; i < structure.languages.length; i += batchSize) {
          const batch = structure.languages.slice(i, i + batchSize);
          console.log(`Processing language batch: ${batch.join(', ')}`);
          
          const batchPromises = batch.map(async (lang) => {
            try {
              const translation = await this.parseLanguageDirectory(owner, repo, `${structure.basePath}/${lang}`);
              return { lang, translation, success: true };
            } catch (error) {
              console.warn(`Failed to parse language ${lang}: ${error.message}`);
              return { lang, translation: {}, success: false };
            }
          });
          
          const batchResults = await Promise.all(batchPromises);
          for (const { lang, translation, success } of batchResults) {
            if (success) {
        ...
        // Optimized batch processing for file-based structure
        console.log(`Processing ${structure.languages.length} language files in batches`);
        const batchSize = 5; // Process 5 files at a time
        
        for (let i = 0; i < structure.languages.length; i += batchSize) {
          const batch = structure.languages.slice(i, i + batchSize);
          console.log(`Processing file batch: ${batch.join(', ')}`);
          
          const batchPromises = batch.map(async (lang) => {
            try {
              const content = await this.github.getFileContent(owner, repo, `${structure.basePath}/${lang}.json`);
              return { lang, translation: this.parseJsonContent(content.content), success: true };
            } catch (error) {
              console.warn(`Failed to load ${lang}.json: ${error.message}`);
              return { lang, translation: {}, success: false };
            }
          });
